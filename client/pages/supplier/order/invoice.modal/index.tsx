...
const InvoiceModal: FC<InvoiceModalProps> = ({
  ...
  return (
    <div
      ...
      tabIndex={-1}
    >
      <div className="modal-dialog modal-dialog-scrollable relative w-4/5 mx-auto pointer-events-none top-10">
          ...
          <div className="relative p-4" ref={printInvoice}>
            <div className="p-8 bg-white text-black">
              ...
              <div className="flex my-2 mt-10">
                <div className="w-40">Customer Name : </div>
                <span className="ml-2 ">
                  {invoice?.supplierInvoice?.customerName}
                </span>
              </div>
              <div className="flex my-2">
                <div className="w-40">Order ID : </div>
                <span className="ml-2">{invoice?.orderId}</span>
              </div>
              <div className="flex my-2">
                <div className="w-40">Customer Email : </div>
                <span className="ml-2">
                  {invoice?.supplierInvoice?.customerEmail}
                </span>
              </div>
              <div className="flex my-2">
                <div className="w-40">Shipping Address : </div>
                <span className="ml-2">
                  {invoice?.supplierInvoice?.shippingAddress}
                </span>
              </div>
              <div className="flex my-2">
                <div className="w-40">Order Date : </div>
                <span className="ml-2">
                  {invoice?.supplierInvoice?.orderDate}
                </span>
              </div>
              {/* <div className="flex my-2">
              ...
              <div className="flex">
                <table className="mx-4 border-collapse border border-2 border-black">
                  <thead>
                    ...
                    <tr>
                      ...
                      {/* <th className="border border-2 border-black p-1">Text</th>
                      <th className="border border-2 border-black p-1">Font</th> */}
                    </tr>
                  </thead>
                  <tbody>
                    {invoice?.supplierInvoice?.products?.map(
                ...
                {/* <table className="mx-4 border-collapse border border-2 border-black">
                  ...
                  <tbody>
                    {invoice?.supplierInvoice?.products?.personalize?.map(
                      (item: any, index: number) => (
                        <tr key={index}>
                          <td className="border border-2 border-black p-1">
                            Yes
              ...
              <div className="grid grid-cols-2 my-2">
                {invoice?.supplierInvoice?.images.map(
