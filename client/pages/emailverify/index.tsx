...
const EmailVerify: NextPage<EmailProps> = ({ products, isLightTheme }) => {
  ...
  useEffect((): void => {
    if (id && token) {
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/customer/email/verify`,
          data
        )
        .then(({ data }): void => {
          console.log(data);
  ...
  const handleRouter = () => {
    if (isUser) router.push("/user/account");
    else router.push("/superadmin/reports");
  };
    ...
    <div className="bg-black h-full min-h-screen flex flex-col font-Raleway container mx-auto">
      ...
      <div className="flex h-max m-auto pt-6 text-white">
        <div className="max-w-xl p-8 text-center text-gray-800 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
          <h3 className="text-2xl">Your Account has been verify</h3>
          ...
          <p>
            We&apos;re happy you&apos;re here. Thanks for signing up for
            personafumes.com!
          </p>
          <div className="mt-4">
            <button
