...
import { AwardModal } from "model/update.landing.modal";
import { joiResolver } from "@hookform/resolvers/joi";
import { AwardSchema } from "schemas/update.landing.schema";

interface AwardSectionProps {
  award: any;
}

const AwardSection: FC<AwardSectionProps> = ({ award }) => {
  ...
  const {
    ...
    resolver: joiResolver(AwardSchema, {
      errors: { wrap: { label: "" } },
      abortEarly: true,
    }),
    defaultValues: {
      detail: award.detail,
      mediaPointer: award.mediaPointer,
  ...
  const handleSectionUpdate = (data: AwardModal) => {
    console.log(data);
    setUpdateBtnClick(true);

    axios
      .put(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/page/award`, data)
      .then(({ data }) => {
        console.log(data);
  ...
  return (
    <div className="py-5 border-b">
      <div className="text-xl font-bold">Award Section</div>
      <form onSubmit={handleSubmit(handleSectionUpdate)}>
        <div className="py-2 flex justify-center items-center">
          <label>Award Section Video : </label>
