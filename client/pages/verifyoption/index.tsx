...
const Register: NextPage<RegisterProps> = ({ products, isLightTheme }) => {
  ...
  const handleEmailSend = () => {
    ...
    if (email) {
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/customer/email/send`,
          data
        )
        .then(({ data }) => {
          console.log(data);
          router.push("/verifyoption/emailsuccess");
        })
        .catch((err) => {
  ...
  const handleOtpSend = () => {
    ...
    if (phone) {
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/customer/otp/send`,
          data
        )
        .then(({ data }) => {
          console.log(data);
          router.push("/verifyoption/otp");
        })
        .catch((err) => {
          console.log(handleAxiosResponseError(err));
          toast.error(handleAxiosResponseError(err));
      ...
      <div className="flex flex-col h-max m-auto pt-6 text-white">
        <div className="max-w-xl p-8 text-center text-gray-800 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
          <h3 className="text-2xl">
            Thanks for signing up for personafumes.com!
          </h3>
          ...
          <p>
            We&apos;re happy you&apos;re here. Let&apos;s get verify yourself:
          </p>
          <div className="mt-4">
            <div className="my-2">
              <button
                type="button"
                className="w-full bg-yellow-400 text-black p-4 rounded font-semibold hover:bg-yellow-300"
