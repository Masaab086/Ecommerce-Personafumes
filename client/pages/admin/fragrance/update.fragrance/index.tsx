...
const UpdateFragrance: React.FC<UpdateProductProps> = ({
  sideBarDisplay,
  setSideBarDisplay,
  setAllFragrances,
  fragrance,
  ...
  const [fragranceValues, setFragranceValues] = useState({
    fragranceName,
    inspiration,
    unitCost,
    unitOfMeasure,
  ...
  useEffect(() => {
    reset(fragranceValues);
  }, [fragrance, reset, fragranceValues]);
  ...
  const handleUpdateProduct = (data: any) => {
    ...
    axios
      .put(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/fragrance/${fragranceId}`,
        data
      )
      .then(({ data }): void => {
        console.log(data);
        setAllFragrances((prev: any) =>
          prev.map((item: any) =>
    ...
    <>
      ...
      >
        <div className="h-[64px] p-1 px-3 flex justify-between items-center">
          <div>
            <div>Update Fragrance</div>
            <div>Update your fragrance and necessary information from here</div>
          </div>
